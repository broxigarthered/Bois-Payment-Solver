...
class NewShopViewController: UIViewController, NSFetchedResultsControllerDelegate {
  ...
  @IBAction func saveNewShoplist(_ sender: Any) {
    guard let shopName = self.newShopName.text,
      let payerName = self.payerTextField.text
      else {
    ...
    if(shopName == ""){
      ...
      self.saveShop(shopName: shopName, payerName: payerName)
      dismiss(animated: true, completion: nil)
    }
  }
  ...
  func saveShop(shopName: String, payerName: String){
    let shop = CoreDataManager.sharedManager.insertShop(shopName: shopName, payerName: payerName)
